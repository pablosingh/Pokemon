...
const createPokemonFn = async (req, res) => {
    ...
    try {
        const pokemonCreted = await Pokemon.create( {
            name, image, hp, attack, defense, speed, height, weight, idApi: 'created'
        } );
        ...
        let pokemonToAddArray = {
            id: pokemonCreted.id,
            types, 
            name, image, hp, attack, defense, speed, height, weight
        };
        cards.push(pokemonToAddArray);
        res.json(pokemonToAddArray);
...
module.exports = createPokemonFn;